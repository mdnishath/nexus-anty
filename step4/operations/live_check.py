...
async def live_check_link(page, share_link, worker_id) -> dict:
    ...
    # Close any popup windows that open during the check (Google auth popups,
    # cookie consent windows, etc.).  Without this handler they stay visible
    # at about:blank and confuse the user.
    ...
    try:
        ...
        # If navigation landed on about:blank the redirect was intercepted
        # (bot-detection, OAuth setup, etc.) — retry with a lower bar.
        if page.url in ('about:blank', ''):
            _log(worker_id, "[LIVE_CHECK] about:blank after goto — retrying")
            try:
                await page.goto(share_link, wait_until="commit", timeout=12000)
            except Exception:
                pass
        ...
        page.remove_listener('popup', _close_popup)
