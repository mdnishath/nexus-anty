...
(function (App) {
    ...
    App.saveConfig = async function (e) {
        let btn = null;
        let origText = '';
        if (e && e.target) {
    ...
    App.setupConfigPanel = function () {
        ...
        const saveBtnH = document.getElementById('saveConfigBtnHeader');
        const reloadBtnH = document.getElementById('reloadConfigBtnHeader');
        if (saveBtn) saveBtn.addEventListener('click', App.saveConfig);
        if (reloadBtn) reloadBtn.addEventListener('click', App.loadConfig);
        if (saveBtnH) saveBtnH.addEventListener('click', App.saveConfig);
        if (reloadBtnH) reloadBtnH.addEventListener('click', App.loadConfig);
    };
