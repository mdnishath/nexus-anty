...
(function (App) {
    ...
    function setupLicenseInfo() {
        ...
        if (deactBtn) deactBtn.addEventListener('click', async () => {
            if (!(await App.confirm('Deactivate this license? You will need to re-enter the key to use the app.', 'Deactivate', 'btn-danger', 'fa-key'))) return;
            try {
        ...
        const doApply = async () => {
            ...
            try {
                const res = await App.apiFetch('/api/license/activate', {
                    method: 'POST',
