...
# Smart wait helper shared with login_flow
try:
    from src.login_flow import _wait_for_any
except ImportError:
    # Fallback stub when imported standalone (e.g. tests)
    async def _wait_for_any(page, selectors, timeout=10000):  # type: ignore[misc]
        await asyncio.sleep(min(timeout / 1000, 3))
...
class ScreenDetector:
    ...
    async def click_try_another_way(self) -> bool:
        ...
        selectors = [
            # Device-challenge (challenge/dp) button — exact match for the
            # Material button Google renders: <button jsname="LgbsSe">…<span
            # jsname="V67aGc">Try another way</span></button>
        ...
        # IMPORTANT: the challenge/dp page is detected by URL the instant it
        # loads, but the "Try another way" button renders a beat later. Looking
        # immediately would miss it (count()==0) and fail the whole login, so
        # wait for the link to actually appear before scanning the selectors.
        try:
            await _wait_for_any(self.page, [
                'button:has-text("Try another way")',
        ...
        for selector in selectors:
            try:
                ...
                if not await element.is_visible():
                    continue  # skip hidden template duplicates
                try:
                    await element.scroll_into_view_if_needed()
                except Exception:
                    pass
                # Robust click: normal → force → JS (the label span can swallow
                # the pointer event, so force/JS are genuine fallbacks here).
                try:
                ...
                logger.info(f"Clicked 'Try another way' via: {selector}")
                await _wait_for_any(self.page, [
                    'input[type="password"]', 'input[name="Passwd"]',
                    '[data-challengetype]', 'input[name="totpPin"]',
        ...
        for selector in selectors:
            try:
                ...
                if await element.count() > 0:
                    ...
                    await _wait_for_any(self.page, [
                        'input[id="backupCodePin"]', 'input[name="backupCode"]',
                        'input[type="tel"]',
                    ], timeout=5000)
                    return True
            except Exception as e:
        ...
        for selector in selectors:
            try:
                ...
                if await element.count() > 0:
                    ...
                    await _wait_for_any(self.page, [
                        'input[type="password"]', 'input[name="Passwd"]',
                        'div[role="navigation"]', '[data-challengetype]',
                    ], timeout=5000)
                    return True
            except Exception as e:
        ...
        for selector in skip_selectors:
            try:
                ...
                if await element.count() > 0 and await element.is_visible():
                    ...
                    await _wait_for_any(self.page, [
                        'div[role="navigation"]', 'div[role="main"]', 'div.aeH',
                        'button:has-text("Skip")', '#identifierId',
                    ], timeout=5000)
                    return True
            except Exception:
                continue
