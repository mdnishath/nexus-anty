...
class LoginBrain:
    ...
    def __init__(
        ...
        self._unknown_last_url: str = ''
        self._bypass_attempts: int = 0  # total _try_bypass calls this session
        self._device_challenge_misses: int = 0  # transient 'Try another way' misses
    ...
    async def _fill_input(self, selectors: list, value: str) -> bool:
        ...
        for sel in selectors:
            try:
                ...
                await self._log(f"  _fill_input: '{sel}' failed — {_fe!s:.80}")
                continue
        return False
    ...
    async def _click_button(self, selectors: list) -> bool:
        ...
        for sel in selectors:
            try:
                ...
                await self._log(f"  _click_button: '{sel}' failed — {_ce!s:.80}")
                continue
        return False
    ...
    async def _click_next(self) -> bool:
        ...
        if not clicked:
            try:
                ...
                await self._log("  _click_next: no button found — pressed Enter as fallback")
                return True
            except Exception as _ke:
                await self._log(f"  _click_next: Enter fallback also failed — {_ke!s:.80}")
        return clicked

    async def _try_bypass(self) -> bool:
        """Click 'Try another way' / 'I don't have my phone'.

        No per-session cap — the post-password polling loop already bounds
        iterations + has stuck-detection, and capping bypass attempts was what
        broke device-challenge → authenticator on accounts that need a couple of
        'Try another way' hops (the mailexus-advanced flow has no such cap)."""
        self._bypass_attempts += 1
        await self._log(f"  _try_bypass: attempt #{self._bypass_attempts} on URL {self.page.url[:90]}")
        result = await self.detector.click_try_another_way()
        await self._log(f"  _try_bypass: click_try_another_way returned {result}")
        return result
    ...
    async def _check_error_container(self) -> Optional[str]:
        ...
        try:
            ...
            await self._log(f"  _check_error_container: inspection failed — {_ee!s:.80}")
        return None
    ...
    async def _wait_for_url_change(self, max_seconds: float = 6.0, interval: float = 0.2) -> bool:
        """Tight poll: return True the moment page.url differs from what it
        was at call time, or False after max_seconds. Use INSTEAD of a fixed
        asyncio.sleep(N) after clicking a navigation button — Google's page
        transitions usually fire in 0.3-1.5s but the old code waited a flat
        3s every time.
        """
    ...
    async def _handle_device_challenge(self) -> HandlerResult:
        ...
        if await self._try_bypass():
            # Tight URL-change poll (was flat 3s sleep)
            self._device_challenge_misses = 0
            await self._wait_for_url_change(max_seconds=4.0)
            return HandlerResult.cont()
        # 'Try another way' wasn't clickable this pass. Don't kill the whole
        # login on one transient miss (the button can render a beat late) —
        # wait and let the polling loop re-detect + retry a couple of times.
        self._device_challenge_misses = getattr(self, '_device_challenge_misses', 0) + 1
        if self._device_challenge_misses < 3:
            await self._log(f"Device challenge — 'Try another way' not ready "
                            f"(miss {self._device_challenge_misses}/3) — waiting + retrying...")
            await self._wait(3)
            return HandlerResult.cont()
        await self._log("WARNING: Could not bypass device challenge after retries!")
    ...
    async def _handle_account_recovery(self) -> HandlerResult:
        ...
        # Tight URL-change poll after clicking the 2FA option (was flat 3s sleep)
        await self._wait_for_url_change(max_seconds=4.0)
        return HandlerResult.cont()
    ...
    async def _handle_unknown(self) -> HandlerResult:
        ...
        # ── Loop prevention: fail after 3 consecutive unknowns on the same URL
        url_key = url.split('?')[0]  # ignore query params for comparison
        if url_key == self._unknown_last_url:
            self._unknown_consecutive += 1
        else:
            self._unknown_consecutive = 1
            self._unknown_last_url = url_key

        if self._unknown_consecutive >= 3:
            return HandlerResult.fail(
                f"LOGIN_TIMEOUT - Stuck on unknown screen after "
                f"{self._unknown_consecutive} attempts at {url_key[:80]}"
            )
        ...
        # Wait and let caller retry (will be caught by loop prevention above)
        await self._wait(3)
        return HandlerResult.cont()
