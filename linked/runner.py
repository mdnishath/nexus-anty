    ...
    async def process_account(self, account):
        ...
        for _proxy_attempt in range(_MAX_PROXY_RETRIES):
            ...
            try:
                ...
                # launch_browser() spawns Chrome via StealthChrome. browser.close()
                # only disconnects Playwright — without stealth.stop() the Chrome
                # process keeps running and accumulates across accounts.
                _stealth = getattr(browser, '_stealth_chrome', None) if browser else None
                _temp_profile = getattr(browser, '_temp_profile', None) if browser else None
                try:
                ...
                if _stealth:
                    try:
                        await asyncio.wait_for(_stealth.stop(), timeout=10)
                    except Exception:
                        try:
                            import subprocess as _sp
                            _p = getattr(_stealth, 'process', None)
                            if _p and _p.pid:
                                _sp.run(['taskkill', '/F', '/T', '/PID', str(_p.pid)],
                ...
                if _temp_profile:
                    try:
                        ...
                        if _os.path.isdir(_temp_profile):
                            _sh.rmtree(_temp_profile, ignore_errors=True)
                    except Exception:
                        pass
                if _socks_bridge:
                ...
                try:
                    _b = locals().get('browser')
                    _stealth = getattr(_b, '_stealth_chrome', None) if _b else None
                    _temp_profile = getattr(_b, '_temp_profile', None) if _b else None
                    if 'context' in locals() and context:
                        await asyncio.wait_for(context.close(), timeout=5)
                    if _b:
                        await asyncio.wait_for(_b.close(), timeout=5)
                    # Kill the real Chrome subprocess — browser.close() won't.
                    if _stealth:
                        try:
                            await asyncio.wait_for(_stealth.stop(), timeout=10)
                        except Exception:
