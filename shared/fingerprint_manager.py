...
def assign(num_workers: int):
    ...
    # Random pool is DESKTOP-only (windows/macos/linux).
    # Android & iOS are honored only when the user explicitly selects them.
    _DESKTOP_POOL = ('windows', 'macos', 'linux')
    _ALL_OS       = tuple(_USER_AGENTS.keys())

    for worker_id in range(1, num_workers + 1):

        # ── OS choice ────────────────────────────────────────────────────────
        if _os_type == 'random':
            os_choice = random.choice(_DESKTOP_POOL)
        elif _os_type in _ALL_OS:
            os_choice = _os_type
        else:
            os_choice = 'windows'
