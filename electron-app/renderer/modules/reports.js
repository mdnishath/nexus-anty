...
(function (App) {
    ...
    App.loadReports = async function () {
        ...
        try {
            ...
            if (!raw.length) {
                rawList.innerHTML = `
                    <div style="padding: 40px; text-align: center; color: #64748b; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.1); margin-bottom: 10px;">
                        <i class="fas fa-file-excel" style="font-size: 32px; opacity: 0.5; margin-bottom: 12px; display: block;"></i>
                        <p style="margin:0 0 16px 0; font-size: 14px;">No output files found.</p>
                        <button class="btn btn-primary btn-sm btn-go-to-profiles"><i class="fas fa-play"></i> Run a Job</button>
                    </div>`;
                const emptyBtn = rawList.querySelector('.btn-go-to-profiles');
                if (emptyBtn) {
    ...
    async function _deleteReport(path, name) {
        if (!(await App.confirm(`Delete report "${name}"? This cannot be undone.`, 'Delete', 'btn-danger', 'fa-trash-alt'))) return;
        try {
    ...
    App.setupDeleteAllReports = function () {
        ...
        btn.addEventListener('click', async () => {
            if (!(await App.confirm('Delete ALL reports? This action cannot be undone.', 'Delete All', 'btn-danger', 'fa-trash-alt'))) return;
