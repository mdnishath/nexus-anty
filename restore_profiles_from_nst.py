...
# Real storage used by the running app (nexus_profile_manager._get_storage_path):
#   %APPDATA%\MailNexusPro\profiles\
# (NOT %LOCALAPPDATA%\GmailBotPro — that's the legacy gmail-bot path.)
STORAGE = Path(os.environ['APPDATA']) / 'MailNexusPro' / 'profiles'
...
def main():
    ...
    print(f"\nOK Wrote {len(local_profiles)} profiles to {PROFILES_FILE}")
