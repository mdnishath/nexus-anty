...
function createWindow() {
    ...
    startBackendProcess()
        .then(async () => {
            ...
            // Poll until Flask responds — exponential backoff (200ms → 2s)
            const http = require('http');
            let delay = 200;
            const maxDelay = 2000;
            const maxAttempts = 30;  // ~30s total with backoff
            for (let i = 0; i < maxAttempts; i++) {
                try {
                    ...
                    if (ok) {
                        console.log(`[Backend] Health check passed after ${i + 1} attempts — notifying renderer`);
                        if (mainWindow && !mainWindow.isDestroyed()) {
            ...
            console.log(`[Backend] Health check never passed after ${maxAttempts} attempts`);
        })
...
// NOTE: before-quit + will-quit already handle full cleanup.
// This is a last-resort fallback — keep it lightweight (no PowerShell).
process.on('exit', () => {
    if (pythonProcess) {
        const pid = _safePid(pythonProcess.pid);
        if (pid) {
