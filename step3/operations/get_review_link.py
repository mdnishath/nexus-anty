...
Flow:
  ...
  5. Click its Share button → _extract_share_url_from_dialog handles
     every popup variant (copy-link-bar span.OT9z9b, legacy readonly
     input, AND the preview-card-only layout where the full URL is read
     from the clipboard after clicking Copy Link)
...
from step3.operations.write_review import _extract_share_url_from_dialog
    ...
    for _share_attempt in range(3):
        try:
            ...
            if share_btn:
                ...
                # Handles EVERY dialog variant: copy-link-bar text
                # (span.OT9z9b), legacy readonly input, the preview-card-
                # only layout (click → read clipboard), and a scoped DOM
                # scan. This is the same robust helper used by write_review,
                # so the link is no longer missed when Google swaps layouts.
                share_link = await _extract_share_url_from_dialog(page, worker_id)
                ...
                if actions_btn:
                    ...
                    for sel in ['li:has-text("Share review")',
                        ...
                        if await el.count() > 0:
                            ...
                            share_link = await _extract_share_url_from_dialog(
                                page, worker_id)
                            await page.keyboard.press('Escape')
                            break
