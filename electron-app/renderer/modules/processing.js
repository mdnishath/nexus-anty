...
(function (App) {
    ...
    App.connectLogStream = function () {
        ...
        let sseUrl = 'http://127.0.0.1:5000/api/log-stream';
    ...
    App.updateProcessButtons = function () {
        ...
        const idleCta = document.getElementById('dashboardIdleCta');
        const progCard = document.getElementById('dashboardProgressCard');
        if (!App.state.serverOnline) {
            if (startBtn) startBtn.disabled = true;
            if (stopBtn) stopBtn.disabled = true;
            if (idleCta) idleCta.style.display = 'none';
        ...
        if (App.state.processing) {
            ...
            if (idleCta) idleCta.style.display = 'block';
            if (progCard) progCard.style.display = 'none';
        }
    };
    ...
    async function _stopProcessing() {
        if (!(await App.confirm('Stop processing?', 'Stop', 'btn-danger', 'fa-stop-circle'))) return;
        try {
