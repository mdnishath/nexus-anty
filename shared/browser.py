...
_IP_CACHE_FILE = os.path.join(tempfile.gettempdir(), 'gmail_bot_ip_cache.json')
_IP_CACHE_TTL = 86400  # 24 hours


def _load_ip_cache():
    """Load IP geo cache from disk (survives restarts)."""
    ...
    try:
        if os.path.exists(_IP_CACHE_FILE):
            ...
            for key, entry in raw.items():
                if now - entry.get('ts', 0) < _IP_CACHE_TTL:
                    _ip_info_cache[key] = tuple(entry['data'])
    except Exception:
        pass


def _save_ip_cache():
    """Persist IP geo cache to disk."""
    try:
        import json as _json
        import time as _time
        now = _time.time()
        raw = {}
        for key, data in _ip_info_cache.items():
            raw[key] = {'data': list(data), 'ts': now}
        with open(_IP_CACHE_FILE, 'w', encoding='utf-8') as f:
            _json.dump(raw, f)
    except Exception:
        pass


# Pre-warm cache from disk on import
_load_ip_cache()

# Country → primary BCP-47 locale (expanded — covers 70+ countries)
_COUNTRY_LOCALE: dict[str, str] = {
    # South Asia
    'BD': 'bn-BD', 'IN': 'hi-IN', 'PK': 'ur-PK', 'LK': 'si-LK',
    'NP': 'ne-NP', 'AF': 'ps-AF',
    # Southeast Asia
    'MM': 'my-MM', 'TH': 'th-TH', 'VN': 'vi-VN', 'LA': 'lo-LA',
    ...
    'UZ': 'uz-UZ', 'KZ': 'kk-KZ', 'GE': 'ka-GE', 'AZ': 'az-AZ',
    'AM': 'hy-AM', 'KG': 'ky-KG',
    # Eastern Europe
    'RU': 'ru-RU', 'UA': 'uk-UA', 'BY': 'be-BY', 'MD': 'ro-MD',
    'PL': 'pl-PL', 'CZ': 'cs-CZ', 'SK': 'sk-SK', 'HU': 'hu-HU',
    'RO': 'ro-RO', 'BG': 'bg-BG', 'RS': 'sr-RS', 'HR': 'hr-HR',
    'SI': 'sl-SI', 'BA': 'bs-BA', 'LT': 'lt-LT', 'LV': 'lv-LV',
    'EE': 'et-EE',
    # Western / Northern Europe
    'DE': 'de-DE', 'AT': 'de-AT', 'CH': 'de-CH', 'FR': 'fr-FR',
    'BE': 'nl-BE', 'IT': 'it-IT', 'ES': 'es-ES', 'PT': 'pt-PT',
    'NL': 'nl-NL', 'SE': 'sv-SE', 'NO': 'nb-NO', 'DK': 'da-DK',
    'FI': 'fi-FI', 'IE': 'en-IE', 'GR': 'el-GR', 'IL': 'he-IL',
    # Middle East
    'TR': 'tr-TR', 'SA': 'ar-SA', 'AE': 'ar-AE', 'EG': 'ar-EG',
    'IQ': 'ar-IQ', 'JO': 'ar-JO', 'LB': 'ar-LB', 'IR': 'fa-IR',
    'QA': 'ar-QA', 'KW': 'ar-KW', 'OM': 'ar-OM', 'BH': 'ar-BH',
    # Americas
    'BR': 'pt-BR', 'MX': 'es-MX', 'AR': 'es-AR', 'CO': 'es-CO',
    'CL': 'es-CL', 'PE': 'es-PE', 'VE': 'es-VE', 'EC': 'es-EC',
    'UY': 'es-UY', 'PY': 'es-PY', 'BO': 'es-BO', 'CR': 'es-CR',
    'DO': 'es-DO', 'GT': 'es-GT', 'PA': 'es-PA',
    # Africa
    'NG': 'en-NG', 'GH': 'en-GH', 'KE': 'en-KE', 'ZA': 'en-ZA',
    'TZ': 'sw-TZ', 'ET': 'am-ET', 'UG': 'en-UG', 'DZ': 'ar-DZ',
    'MA': 'ar-MA', 'TN': 'ar-TN', 'SN': 'fr-SN', 'CI': 'fr-CI',
    'CM': 'fr-CM', 'CD': 'fr-CD', 'MG': 'mg-MG',
    # English-speaking
...
def _lookup_ip_info(proxy: dict | None) -> tuple[str, str, float, float]:
    ...
    Results are cached in memory AND persisted to disk (24h TTL) so repeated
    launches of the same proxy skip the HTTP round-trip entirely.
    """
    ...
    if data.get('status') == 'success':
        ...
        _save_ip_cache()  # Persist so next restart skips HTTP call
        return result

    # Proxy lookup failed — fall back to local IP detection
    if proxy:
        server = proxy.get('server', 'unknown')
        print(f"[GEO] Proxy lookup failed for {server}. Google might see your real IP! Fallback to local detection...")
        try:
...
_PRUNE_DIRS = [
    ...
    'CacheStorage', 'DawnCache', 'DawnWebGPUCache',
    'Crashpad', 'BrowserMetrics',
]
# DELIBERATELY EXCLUDED (session-critical — deleting these causes logout):
#   'Service Worker'  — Gmail PWA registration, offline sync
#   'blob_storage'    — session-related blob data
#   'IndexedDB'       — Google stores OAuth tokens here


def _prune_profile(profile_path: str):
    """Delete known cache subdirs inside a Chrome profile to reclaim disk space.
    Only runs if the profile dir exceeds 100 MB to avoid unnecessary I/O."""
    # Quick size estimate — sum top-level entries (fast, avoids deep walk)
    try:
        total_size = 0
        _SIZE_THRESHOLD = 100 * 1024 * 1024  # 100 MB
        for entry in os.scandir(profile_path):
            if entry.is_file(follow_symlinks=False):
                total_size += entry.stat().st_size
            elif entry.is_dir(follow_symlinks=False):
                # Rough estimate: sum first-level files in each subdir
                try:
                    ...
                    pass
            if total_size >= _SIZE_THRESHOLD:
                break  # already over threshold, no need to keep counting
        if total_size < _SIZE_THRESHOLD:
            return  # Profile is small enough — skip pruning
    except Exception:
        pass  # If size check fails, prune anyway as a safety fallback
...
async def _launch_nst_context(playwright, proxy=None, worker_id=None,
    ...
    # ── Popup wake handler ─────────────────────────────────────────────────
    # When Playwright connects via connect_over_cdp, Chrome pauses new popup
    # targets in "waiting for debugger" state. Without this handler any
    # popup opened by window.open() (e.g. "Continue with Google" on Reddit)
    # stays at about:blank permanently.
    async def _on_new_page(new_page):
        for attempt in range(6):
            try:
                await asyncio.sleep(0.05 + (0.1 * attempt))
                if new_page.is_closed(): return
                ...
                if attempt == 5:
                    print(f"[BROWSER] NST popup wake failed after 6 tries")
...
async def _launch_persistent_context(
    ...
    # NOTE: Do NOT clear cookies — persistent profiles must keep their
    # session data (Gmail login, etc.) across browser restarts.
