...
class GmailBotWorker(BaseGmailBotWorker):
    ...
    async def process_batch_live_check(self, excel_processor, stop_event):
        ...
        try:
            ...
            # Close any extra pages/popups that open (Google auth windows, cookie consent,
            # etc.) — they would otherwise sit at about:blank and stay visible.
            ...
            # launch_browser() spawned a real Chrome subprocess via StealthChrome.
            # browser.close() only disconnects Playwright — the underlying Chrome
            # process keeps running unless we call stealth.stop(). Without this,
            # browsers pile up after each batch run and crash the laptop.
            ...
            if temp_profile:
                try:
                    ...
                    if _os.path.isdir(temp_profile):
                        _sh.rmtree(temp_profile, ignore_errors=True)
                except Exception:
                    pass
            if socks_bridge:
                try:
