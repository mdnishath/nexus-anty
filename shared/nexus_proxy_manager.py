...
def check_proxy(proxy: dict, timeout: int = 15) -> dict:
    ...
    try:
        ...
        # Map proxy type → URL scheme. socks5h = SOCKS5 with remote DNS (matches browser).
        if ptype in ("socks5", "socks5h"):
            scheme = "socks5h"
        elif ptype == "socks4":
            scheme = "socks4"
        else:
            scheme = "http"  # 'http' and legacy 'https' both tunnel via HTTP CONNECT

        auth = f"{user}:{pwd}@" if (user and pwd) else ""
        proxy_url = f"{scheme}://{auth}{host}:{port}"

        # requests supports socks5 via PySocks (installed) — one code path for both.
