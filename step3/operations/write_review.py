...
# Strict pattern: ONLY accept genuine Google Maps share URLs.
# Anchored with ^...$ so wrappers like 'whatsapp://send?text=https://maps...'
# or 'twitter.com/intent/tweet?url=...' can NEVER pass — the entire input
# must be a clean Maps URL after strip().
...
async def _clear_clipboard(page) -> None:
    """
    Best-effort: blank the clipboard so a STALE URL from a previous
    review's copy can never be mistaken for THIS dialog's link.
    Needs clipboard-write permission (granted at context launch).
    """
    try:
        await page.evaluate(
...
async def _read_clipboard_url(page) -> str:
    """Read the clipboard; return it only if it is a clean Maps URL, else ''."""
    try:
        ...
        return ''
    clip = (clip or '').strip()
    return clip if _MAPS_URL_RE.fullmatch(clip) else ''


async def _extract_share_url_from_dialog(page, worker_id) -> str:
    """
    Robustly read the share URL from the Share dialog, handling BOTH
    layout variants Google A/B-tests (so the link is never missed):

      VARIANT A — "copy-link bar": the URL is visible as TEXT in
        <div class="KKCW6e yWlric">
          <span class="OT9z9b">https://maps.app.goo.gl/XXXX</span>
          <button id="copy-link-bar-copy-button" ...>Copy Link</button>
        </div>

      VARIANT B — "preview card only": there is NO visible URL text — just
        a preview card
            <div class="whdmFb" data-skw-id="preview">
              <div class="pAvKO ...">... <span>maps.app.goo.gl</span> ...</div>
            </div>
        plus a row of share targets. Only the truncated host
        'maps.app.goo.gl' is shown; the FULL URL is obtainable solely by
        triggering a copy and reading the clipboard.

    Strategy — returns at the FIRST regex-validated Maps URL, so a missing
    OT9z9b span (Variant B) no longer means a missing link:
      1. span.OT9z9b visible text (Variant A — no click).
      2. Legacy readonly <input> value.
      3. Clear clipboard, then click a SAFE copy affordance (stable id /
         preview card / aria-labelled Copy Link — NEVER WhatsApp/Gmail/
         Embed) and read the clipboard. Handles Variant B.
      4. DOM scan scoped to the copy-link bar.

    Validates every candidate with _MAPS_URL_RE (fullmatch) so a
    WhatsApp/Gmail/embed deep-link can never pass. Does NOT close the
    dialog — the caller presses Escape.
    """
    # 1) Wait for ANY share-dialog anchor — not ONLY span.OT9z9b, which
    #    Variant B never renders. First match that appears wins the wait.
    for sel in ['span.OT9z9b', '[data-skw-id="preview"]',
                'button#copy-link-bar-copy-button', 'div[role="dialog"]']:
        try:
    ...
    # ── 2) Variant A: span.OT9z9b visible text (no click needed) ──────────
    for sel in ['div.KKCW6e span.OT9z9b', 'span.OT9z9b']:
        try:
            sp = page.locator(sel).first
            if await sp.count() == 0:
                continue
            for getter_name in ('inner_text', 'text_content'):
                try:
                    raw = (await getattr(sp, getter_name)() or '').strip()
                except Exception:
                    continue
                if _MAPS_URL_RE.fullmatch(raw):
                    _log(worker_id,
                         f'[SHARE] Validated URL from {sel} ({getter_name}): {raw}')
                    return raw
                elif raw:
                    _log(worker_id,
                         f'[SHARE] Rejected non-Maps text from {sel}: {raw[:80]!r}')
        except Exception:
            continue

    # ── 3) Legacy readonly <input> ────────────────────────────────────────
    for inp_sel in ['input.vrsrZe[readonly]',
                    'input[readonly][value*="maps"]',
                    'input[readonly][value*="goo.gl"]',
        ...
        try:
            ...
            if _MAPS_URL_RE.fullmatch(val):
                _log(worker_id, f'[SHARE] Validated URL from legacy input: {val}')
                return val
        except Exception:
            continue

    # ── 4) Variant B: click a SAFE copy affordance, then read clipboard ──
    #    Clear the clipboard FIRST so a stale URL can't be read back as a
    #    false positive. We ONLY click link-copy affordances — never the
    #    WhatsApp/Gmail/Embed targets (generic div.r3L9Lb), which would
    #    copy a deep-link or open an external app. aria-label is matched
    #    case-insensitively by language root ("copy"/"copier"/"kopieren"/
    #    "copia"/"copiar") — none of which match WhatsApp/Gmail/Embed.
    await _clear_clipboard(page)
    copy_selectors = [
        # Explicit, known-safe "Copy Link" affordances FIRST. aria-label is
        # matched case-insensitively by language root ("copy"/"copier"/
        # "kopieren"/"copia"/"copiar") — none match WhatsApp/Gmail/Embed.
        'button#copy-link-bar-copy-button',           # Variant A bar (stable id)
        '[role="button"][aria-label*="copy" i]',      # Copy Link / Copy link
        '[role="button"][aria-label*="copier" i]',    # French
        '[role="button"][aria-label*="kopieren" i]',  # German
        '[role="button"][aria-label*="copia" i]',     # Italian
        '[role="button"][aria-label*="copiar" i]',    # Spanish / Portuguese
        'button[aria-label*="copy" i]',
        'button[aria-label*="copier" i]',
        'button.oucrtf',
        # Last resort: the Variant B preview card itself acts as a copy
        # button. Tried last in case any layout treats it as navigation.
        'div.pAvKO',
        '[data-skw-id="preview"]',
    ]
    for cp_sel in copy_selectors:
        try:
            cp = page.locator(cp_sel).first
            if (await cp.count() == 0) or not (await cp.is_visible()):
                continue
            await cp.click()
            await asyncio.sleep(1.2)
            url = await _read_clipboard_url(page)
            if url:
                _log(worker_id, f'[SHARE] Validated URL via clipboard ({cp_sel}): {url}')
                return url
        except Exception:
            continue

    # ── 5) Last-resort DOM scan — scoped STRICTLY to the copy-link bar
    #    (div.KKCW6e). We do NOT scan the whole dialog because WhatsApp/
    #    Gmail/embed anchors embed Maps URLs in their hrefs and would
    #    poison the result.
    try:
        url_from_dom = await page.evaluate(r"""
            () => {
                const bar = document.querySelector('div.KKCW6e');
                if (!bar) return '';
                const txt = (bar.innerText || bar.textContent || '').trim();
                const re = /^https:\/\/(?:maps\.app\.goo\.gl\/[A-Za-z0-9_-]+|goo\.gl\/maps\/[A-Za-z0-9_-]+|www\.google\.[a-z.]+\/maps\/[^\s'"<>]+)(?:[?#][^\s'"<>]*)?$/i;
                return re.test(txt) ? txt : '';
            }
        """)
        url_from_dom = (url_from_dom or '').strip()
        if url_from_dom and _MAPS_URL_RE.fullmatch(url_from_dom):
            _log(worker_id, f'[SHARE] Validated URL via DOM scan: {url_from_dom}')
            return url_from_dom
    except Exception as e:
        _log(worker_id, f'[SHARE] DOM scan error: {e}')
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if share_btn:
                ...
                # The helper handles EVERY dialog variant: copy-link-bar
                # text (span.OT9z9b), legacy readonly input, the preview-
                # card-only layout (click → clipboard), and a scoped DOM
                # scan — so the link is not missed when Google swaps the
                # popup layout.
                val = await _extract_share_url_from_dialog(page, worker_id)
                if val:
                        ...
                        if await actions_btn.count() > 0:
                            ...
                            for sh_sel in [
                                ...
                                if await el.count() > 0:
                                    await el.click()
                                    await asyncio.sleep(4)
                                    # Same all-variant helper as Method 1.
                                    txt = await _extract_share_url_from_dialog(page, worker_id)
                                    if txt:
                                        await page.keyboard.press('Escape')
                                        _log(worker_id, f'[SHARE] Link via menu: {txt[:80]}')
                                        return txt
                                    await page.keyboard.press('Escape')
                                    break
                            else:
...
async def write_review(page, worker_id, place_url: str,
    ...
    try:
        ...
        if not text:
            ...
            # ── Strong retry: review is LIVE but share link missing.
            #     Google sometimes lags ~60s before the Share button shows
            #     up on a newly-live review. Wait ~1 min, reload contrib
            #     reviews tab, retry extraction. Up to 3 rounds.
            #
            #     HARD TIMEOUT: each round is capped at 2 minutes. If any
            #     await inside the round hangs (frozen tab, stuck network
            #     I/O, browser dead) the wait_for raises TimeoutError and
            #     we move on to the next round which begins with a reload.
            #     This protects against the "stuck for 40 min" scenario.
            if not share_link and review_status in ('live', 'posted'):
                async def _one_share_retry_round(rnd: int):
                    nonlocal share_link
                    _log(worker_id,
                         f'[WRITE_REVIEW] Live but no share link — '
                         f'waiting 60s before retry round {rnd}/3...')
                    ...
                    try:
                        ...
                        _log(worker_id,
                             f'[WRITE_REVIEW] reload error round {rnd}: {_re}')
                    await asyncio.sleep(3)
                    try:
                        for tab_sel in [
                            'button[role="tab"]:has-text("Reviews")',
                    ...
                    share_link = await _get_share_link(
                        page, worker_id, max_attempts=2)

                for _round in range(1, 4):
                    try:
                        # 2 min cap per round: 60s sleep + reload + retry
                        # extraction. If anything hangs past this, the
                        # round is abandoned and the next one begins.
                        await asyncio.wait_for(
                            _one_share_retry_round(_round), timeout=150
                        )
                    except asyncio.TimeoutError:
                        _log(worker_id,
                             f'[WRITE_REVIEW] Round {_round} hit 2-min '
                             f'hard timeout — abandoning and moving on')
                    except Exception as _rr_err:
                        _log(worker_id,
                             f'[WRITE_REVIEW] Round {_round} error: {_rr_err}')
                    if share_link:
                        _log(worker_id,
                             f'[WRITE_REVIEW] Share link captured on '
                             f'round {_round}: {share_link[:80]}')
                        break

            if not share_link:
                _log(worker_id,
                     '[WRITE_REVIEW] WARNING: Could not obtain share link '
                     'after all retries — review is still POSTED, sheet '
                     'will be updated WITHOUT Review Live Link (user can '
                     'fill it manually later)')

        # ── Build result ────────────────────────────────────────────────────
        # A review is POSTED whenever the status reached live/posted, even
        # if we failed to capture the share link (network glitch, browser
        # crash, etc). The Google Sheet will still be updated — only the
        # Review Live Link cell will be left blank in that case so the
        # user can paste it manually later.
        is_posted = review_status in ('live', 'posted')
        result.update({
            'success':             is_posted,
