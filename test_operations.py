...
async def _remove_existing_item(page, worker_id) -> bool:
    ...
    """
    W = f"[OPS][W{worker_id}]"
    # NOTE: must be a :is() group, not a comma-list. A plain
    # 'main, [role="main"] button' is CSS "main OR [role=main] button",
    # which makes `.first` match the <main> element itself instead of the
    # button inside it — Playwright then clicks the whole main block and
    # nothing happens.
...
async def _phone_exists_on_page(page) -> bool:
    """Phone is set iff the Add button is gone. Google's rescuephone page
    renders ONE of two states in main:
       no phone    → <button jsname="Pr7Yme"> Add recovery phone </button>
       phone set   → <li> with the number + Edit/Remove buttons
    Detecting via "is Add button visible?" is more reliable than scanning
    for <li>, since Google's header/footer can contain unrelated <li>s
    (apps menu, etc.) that would false-positive."""
    add_sels = [
        ':is(main, [role="main"]) button[jsname="Pr7Yme"]',
        ':is(main, [role="main"]) button[data-idom-class="wMI9H"]',
    ...
    for sel in add_sels:
        try:
            ...
            if await elem.count() > 0 and await elem.is_visible():
                return False  # Add button visible → no phone yet
        except Exception:
            continue
    # No Add button found → phone must already exist (li layout) OR page
    # didn't load. Fall back to the old <li>-in-main check.
    for sel in [':is(main, [role="main"]) li']:
        try:
            elem = page.locator(sel).first
            if await elem.count() > 0 and await elem.is_visible():
                return True
...
async def _open_phone_dialog(page, worker_id) -> bool:
    ...
    # Add mode — LANGUAGE-AGNOSTIC selectors first (the button label changes
    # with the account's Google language/country, so anchoring on the text
    # like "Add recovery phone" breaks for any non-EN account).
    # Stable Google attributes on the Add button:
    #   <button jsname="Pr7Yme" jscontroller="O626Fe" data-idom-class="wMI9H" ...>
    # We scope to main/[role=main] so we don't grab a header/nav button that
    # happens to share these attrs.
    add_sels = [
        f'{MAIN} button[jsname="Pr7Yme"]',
        f'{MAIN} button[data-idom-class="wMI9H"]',
        f'{MAIN} button[jscontroller="O626Fe"]',
        'button[jsname="Pr7Yme"]',
        'button[data-idom-class="wMI9H"]',
        # Text-based fallbacks — only useful if Google ever rebrands the
        # button. Kept last so language differences never block the op.
        f'{MAIN} > button',
        f'{MAIN} button >> nth=0',
...
async def remove_recovery_email(page, account: dict, worker_id: int):
    ...
    try:
        ...
        # NOTE: must be a :is() group, not a comma-list. A plain
        # 'main, [role="main"] button' is CSS "main OR [role=main] button",
        # which makes `.first` match the <main> element itself instead of
        # the button inside it — Playwright then clicks the whole main
...
async def _email_exists_on_page(page) -> bool:
    """Email is set iff the Add button is gone. Same logic as
    _phone_exists_on_page — detect via Add button visibility rather than
    counting <li>s, since unrelated <li>s elsewhere on the page (apps
    menu, footer, etc.) would otherwise false-positive."""
    add_sels = [
        ':is(main, [role="main"]) button[jsname="Pr7Yme"]',
        ':is(main, [role="main"]) button[data-idom-class="wMI9H"]',
        'button[jsname="Pr7Yme"]',
...
async def _open_email_dialog(page, worker_id) -> bool:
    ...
    # Add mode — LANGUAGE-AGNOSTIC selectors first. The label changes with
    # the account's Google language so we anchor on stable Google attrs:
    #   <button jsname="Pr7Yme" jscontroller="O626Fe" data-idom-class="wMI9H" ...>
    # (same component as the recovery-phone Add button)
    add_sels = [
        f'{MAIN} button[jsname="Pr7Yme"]',
        f'{MAIN} button[data-idom-class="wMI9H"]',
        f'{MAIN} button[jscontroller="O626Fe"]',
        'button[jsname="Pr7Yme"]',
        'button[data-idom-class="wMI9H"]',
        # Text-based fallbacks last — only matter if Google rebrands.
        f'{MAIN} > button',
        f'{MAIN} button >> nth=0',
...
async def change_name(page, account: dict, worker_id: int) -> bool | str:
    """
    Op 8 — Change account display name (First Name + Last Name).

    Google profile name page DOM (may vary by locale):
      - Input for first name: input[aria-label="First name"], input[name="firstName"]
      - Input for last name: input[aria-label="Last name"], input[name="lastName"]
      - Save button: button[type="submit"]:has-text("Save"), button:has-text("Save")
    """
    W = f"[OPS][W{worker_id}]"
    first_name = str(account.get('First Name', '')).strip()
    last_name = str(account.get('Last Name', '')).strip()

    if not (first_name or last_name):
        _log(f"{W} [OP8] No first/last name provided")
        return 'SKIP - No name provided'

    try:
        # ── A: Navigate ──────────────────────────────────────────────────
        _log(f"{W} [OP8][A] Navigating to name change page...")
        await robust_goto(page, NAME_URL, worker_id=worker_id)
        await asyncio.sleep(2)

        # ── B: Handle reauth ─────────────────────────────────────────────
        _log(f"{W} [OP8][B] Checking for reauth...")
        reauth_ok = await handle_reauth(page, account, worker_id)
        if not reauth_ok:
            _log(f"{W} [OP8][B] Reauth failed — cannot proceed")
            return False
        ...
        # After reauth, Google may redirect. Navigate back if needed.
        current_url = page.url.lower()
        if 'profile/name' not in current_url:
            _log(f"{W} [OP8][B] Redirected — navigating back to name page...")
            await robust_goto(page, NAME_URL, worker_id=worker_id)
            await asyncio.sleep(2)

        # ── C+D: Locate both name inputs ─────────────────────────────────
        # Google profile name page: both inputs share jsname="YPqjbf".
        # Order matters: nth(0)=First name, nth(1)=Last name. Language-agnostic.
        first_input = None
        last_input = None
        try:
            jsname_inputs = page.locator('input[jsname="YPqjbf"]')
            jsn_count = await jsname_inputs.count()
            if jsn_count >= 2:
                first_input = jsname_inputs.nth(0)
                last_input = jsname_inputs.nth(1)
                _log(f"{W} [OP8][C] Located inputs via jsname='YPqjbf' (count={jsn_count})")
        except Exception:
            pass

        # Fallback to legacy attribute-based selectors if jsname didn't yield 2 inputs
            ...
            for sel in first_sels:
                try:
                    ...
                    if await cand.count() > 0 and await cand.is_visible():
                        first_input = cand
                        _log(f"{W} [OP8][C] Located first input via fallback: {sel}")
                        break
                except Exception:
                    continue
            for sel in last_sels:
                try:
                    cand = page.locator(sel).first
                    if await cand.count() > 0 and await cand.is_visible():
                        last_input = cand
                        _log(f"{W} [OP8][D] Located last input via fallback: {sel}")
                        break
                except Exception:
                    continue

        if first_input is None:
            _log(f"{W} [OP8][C] FAILED — could not find first name input")
            return False
        if last_input is None:
            _log(f"{W} [OP8][D] FAILED — could not find last name input")
            return False

        # ── C: Fill first name ───────────────────────────────────────────
        _log(f"{W} [OP8][C] Filling first name: {first_name}")
        try:
            await first_input.click()
            await asyncio.sleep(0.3)
            await first_input.fill('')
            await first_input.type(first_name, delay=50)
            _log(f"{W} [OP8][C] First name filled")
            await asyncio.sleep(1)
        except Exception as e:
            _log(f"{W} [OP8][C] FAILED to fill first name: {e}")
            return False

        # ── D: Fill last name ─────────────────────────────────────────────
        _log(f"{W} [OP8][D] Filling last name: {last_name}")
        try:
            await last_input.click()
            await asyncio.sleep(0.3)
            await last_input.fill('')
            await last_input.type(last_name, delay=50)
            _log(f"{W} [OP8][D] Last name filled")
            await asyncio.sleep(1)
        except Exception as e:
            _log(f"{W} [OP8][D] FAILED to fill last name: {e}")
            return False

        # ── E: Click Save ─────────────────────────────────────────────────
        _log(f"{W} [OP8][E] Clicking Save...")
        save_sels = [
            'button[type="submit"]',
        ...
        for sel in save_sels:
            try:
                ...
                if await btn.count() > 0 and await btn.is_visible():
                    await btn.scroll_into_view_if_needed()
                    await btn.click()
                    _log(f"{W} [OP8][E] Clicked save via: {sel}")
                    await asyncio.sleep(3)
                    # Check for success - should redirect or show confirmation
                    saved = True
                    break
            except Exception:
                continue
        ...
        if not saved:
            _log(f"{W} [OP8][E] FAILED — could not click save button")
            return False

        # ── F: Verify ─────────────────────────────────────────────────────
        _log(f"{W} [OP8][F] Verifying name change...")
        # After save, we should be on a different page or see confirmation
        # Check if we're still on the name page (might indicate error)
        current_url = page.url.lower()
        if 'profile/name' in current_url:
            _log(f"{W} [OP8][F] Still on name page — checking for errors...")
            # Check for error messages
            error_sels = ['div[role="alert"]', '.error', '[aria-live="polite"]']
            for sel in error_sels:
                try:
                    ...
                    if await err.count() > 0 and await err.is_visible():
                        ...
                        _log(f"{W} [OP8][F] Error detected: {err_text[:100]}")
                        return f"FAILED: {err_text[:100]}"
                except Exception:
                    pass
            return False

        _log(f"{W} [OP8] SUCCESS — Name changed to {first_name} {last_name}")
        return True

    except Exception as e:
        _log(f"{W} [OP8] EXCEPTION: {e}", 'error')
