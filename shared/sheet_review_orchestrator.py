...
# Aliases for the destination-URL column. The user's Project_Management
# sheet calls it "Review Post Link"; older templates use "Direct Review Link".
DIRECT_LINK_ALIASES = ('direct review link', 'review post link')


def _find_header_row(values: list[list]) -> Optional[int]:
    """Locate the row that contains 'Review Text' and the destination
    review URL column (Direct Review Link or Review Post Link).
    ...
    for ri, row in enumerate(values):
        ...
        if 'review text' in cells and any(
            alias in cells for alias in DIRECT_LINK_ALIASES
        ):
            return ri
    return None
...
def parse_business_tab(resources_path, sheet_id: str, tab_name: str) -> dict:
    ...
    for needed in HEADERS_REQUIRED:
        ...
        if key == 'order #' and ('order #' in cols or 'order#' in cols):
            continue
        # Tolerate "Direct Review Link" / "Review Post Link" alias
        if key == 'direct review link' and any(a in cols for a in DIRECT_LINK_ALIASES):
            continue
        if key not in cols:
    ...
    direct_c = next(
        (cols[a] for a in DIRECT_LINK_ALIASES if a in cols),
        None,
    )
    ...
    for offset, row in enumerate(values[h_idx + 1:], start=h_idx + 2):
        ...
        # A post mid-flight — leave it alone.
        if status in ('pending', 'queued', 'in progress'):
            pending += 1
            continue

        # Eligible rows must be completely clean: Date, Review Live Link
        # and Status all empty. The moment ANY of them is filled the row
        # is treated as already posted / handled, so the op never re-posts
        # or overwrites it — this covers rows posted manually with a blank
        # Status, and rows the Appeal flow marked 'Appealed'/'Missing'/
        # 'Done'. Notes is intentionally NOT checked: on a clean row it
        # carries the desired star count (1-5; empty → 5 stars).
        if date_v or live_link or status:
            posted += 1
            continue
...
def _parse_tabs_parallel(resources_path, sheet_id: str,
                         tab_names: list[str], max_workers: int = 10) -> dict:
    """Read N tabs in parallel via ThreadPoolExecutor. Returns
    {tab_name: parse_business_tab_result} in input order.
    Google Sheets API is IO-bound so threads give a big speedup."""
    from concurrent.futures import ThreadPoolExecutor
    if not tab_names:
        return {}
    workers = max(1, min(max_workers, len(tab_names)))
    results: dict = {}
    with ThreadPoolExecutor(max_workers=workers) as ex:
        future_to_tab = {
            ex.submit(parse_business_tab, resources_path, sheet_id, t): t
            for t in tab_names
        }
        for fut in future_to_tab:
            t = future_to_tab[fut]
            try:
                results[t] = fut.result()
            except Exception as e:
                results[t] = {'success': False, 'message': f'Parse error: {e}'}
    return results
...
def plan_run(resources_path, sheet_id: str,
    ...
    """
    valid_cfgs = [c for c in tabs_config
                  if (c.get('tab_name') or c.get('tab')) and
                  max(0, int(c.get('count') or 0)) > 0]
    tab_names = [c.get('tab_name') or c.get('tab') for c in valid_cfgs]
    parsed = _parse_tabs_parallel(resources_path, sheet_id, tab_names)

    out = []
    total = 0
    for cfg in valid_cfgs:
        tab = cfg.get('tab_name') or cfg.get('tab')
        count = max(0, int(cfg.get('count') or 0))
        info = parsed.get(tab) or {'success': False, 'message': 'Missing tab data'}
