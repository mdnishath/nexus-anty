...
(function (App) {
    ...
    App.checkBackendStatus = async function () {
        try {
            const response = await App.apiFetch('/api/health', {
                method: 'GET',
    ...
    App.startAggressiveDetection = async function () {
        ...
        for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
                const res = await App.apiFetch('/api/health', { method: 'GET' });
                if (res.ok) {
    ...
    App.setServerOnline = async function () {
        // Ensure API token is available before making any authenticated calls
        if (!App.state.apiToken && App.fetchApiToken) {
