...
class BaseGmailBotWorker:
    ...
    async def _cleanup_browser(self, context=None, browser=None,
        ...
        launch_browser() spawns a real Chrome subprocess via StealthChrome and
        attaches it to browser._stealth_chrome. browser.close() only disconnects
        Playwright over CDP — it does NOT kill the Chrome process. Without
        stealth.stop() the Chrome process leaks; many parallel workers pile up
        and crash the laptop.
        """
        # Capture stealth + temp profile BEFORE browser.close() (CDP detach)
        stealth = getattr(browser, '_stealth_chrome', None) if browser else None
        temp_profile = getattr(browser, '_temp_profile', None) if browser else None

        try:
            if context:
                await asyncio.wait_for(context.close(), timeout=5)
        except Exception as _ce:
            _log(self.worker_id, f"[CLEANUP] context.close() failed: {_ce!s:.80}")
        try:
            if browser:
                await asyncio.wait_for(browser.close(), timeout=5)
        except Exception as _be:
            _log(self.worker_id, f"[CLEANUP] browser.close() failed: {_be!s:.80}")

        # CRITICAL: kill the real Chrome subprocess (browser.close() won't).
        if stealth:
            try:
                await asyncio.wait_for(stealth.stop(), timeout=10)
            except Exception as _se:
                _log(self.worker_id, f"[CLEANUP] stealth.stop() failed: {_se!s:.80}")
                # Fallback: hard-kill by PID so the process never lingers.
                try:
                    proc = getattr(stealth, 'process', None)
                    if proc and proc.pid:
                        subprocess.run(
                            ['taskkill', '/F', '/T', '/PID', str(proc.pid)],
                            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                        )
                        _log(self.worker_id, f"[CLEANUP] Force-killed Chrome PID {proc.pid}")
                except Exception:
                    pass

        # launch_browser() creates a fresh temp profile dir per call — remove it
        ...
        if socks_bridge:
            try:
                ...
                _log(self.worker_id, f"[CLEANUP] socks_bridge.stop() failed: {_se!s:.80}")
        if pw:
            try:
                await pw.stop()
            except Exception as _pe:
                _log(self.worker_id, f"[CLEANUP] playwright.stop() failed: {_pe!s:.80}")
    ...
    async def _process_single_attempt(self, account, email, row_index,
        ...
        try:
            async with async_playwright() as p:
                ...
                await self._cleanup_browser(context, browser, socks_bridge)
                _log(self.worker_id, "[BROWSER] Closed")
            ...
            # async_playwright context manager handled pw.stop(); but the Chrome
            # subprocess inside StealthChrome only dies if we call stealth.stop().
            _locals = locals()
            await self._cleanup_browser(
                context=_locals.get('context'),
                browser=_locals.get('browser'),
                socks_bridge=_locals.get('socks_bridge'),
            )
            _log(self.worker_id, "[BROWSER] Closed")
